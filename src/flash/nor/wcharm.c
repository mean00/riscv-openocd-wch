...
static int ch32x_protect_check(struct flash_bank *bank)
{
	...
	for (int i = 0; i < (int)bank->num_prot_blocks; i++)
...
{	
    ...
    uint32_t tmp;
	int retval = target_read_u32(target, 0x1ffff7e0, &tmp);	
	if (retval != ERROR_OK)
		return retval;
	*flash_size_in_kb = tmp;
	return retval;
}
...
int get_ch32x_info(struct flash_bank *bank, struct command_invocation *cmd)
{
...
COMMAND_HANDLER(ch32x_handle_mass_erase_command)
{
	...
	if (retval == ERROR_OK) {
		...
		for (i = 0; i < (int)bank->num_sectors; i++)
